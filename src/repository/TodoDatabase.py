...
class TodoDatabase:
    ...
    def add(self, taskObject):
        self.connection.execute("INSERT into tasks values (?, ?, ?, ?)", (taskObject.name(), taskObject.done(), taskObject.listname(), taskObject.is_visible()))
    ...
    def retrieve(self):
        result = []
        for taskArray in self.connection.execute("SELECT name, isDone, listname, visible FROM tasks"):
            result.append(Task(taskArray[0], taskArray[2], taskArray[1], taskArray[3]))
        return result
